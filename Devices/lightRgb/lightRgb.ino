...
#define FIRMWARE_VERSION "Example-lightRgb"  // Custom name for this program
...
int splitColor(char* color, char* tokensC[]);
...
void callback(char* topic, byte* payload, unsigned int length) {  //A new message has been received
	...
	} else if (strcmp(tokens[1], "directive") == 0 && strcmp(tokens[2], "color") == 0) {
		char* tokensC[3];
		splitColor(message, tokensC);
		int r = atoi(tokensC[0] + 4);
		int g = atoi(tokensC[1]);
		int b = atoi(tokensC[2]);
...
int splitColor(char* color, char* tokensC[]) {
	const char s[2] = ",";
	int pos = 0;
	tokensC[0] = strtok(color, s);

	while (pos < 3 - 1 && tokensC[pos] != NULL) {
		pos++;
		tokensC[pos] = strtok(NULL, s);
	}
...
void setColor(int r, int g, int b) {
	...
	char msg[18];
	sprintf(msg, "rgb(%d,%d,%d)", red, green, blue);
