...
#define FIRMWARE_VERSION "Example-lightGroup"  // Custom name for this program
...
void callback(char* topic, byte* payload, unsigned int length) {  //Callback for new messages
	...
	if (strcmp(tokens[1], "directive") == 0) {
		...
		} else if (strcmp(tokens[2], "percentage1") == 0) {
			int val = atoi(message);
			if (val >= 0 && val <= 100) {
				setBrightness(1,val);
			}
		} else if (strcmp(tokens[2], "percentage2") == 0) {
			int val = atoi(message);
			if (val >= 0 && val <= 100) {
				setBrightness(2,val);
			}
		} else if (strcmp(tokens[2], "percentage3") == 0) {
			int val = atoi(message);
			if (val >= 0 && val <= 100) {
				setBrightness(3,val);
			}
		} else if (strcmp(tokens[2], "percentage4") == 0) {
			int val = atoi(message);
			if (val >= 0 && val <= 100) {
				setBrightness(4,val);
			}
...
void setBrightness(int index, int value) {
	Serial.printf("Percentage %d set to %d\n", index, value);
	char val[4];
	sprintf(val, "%d",value);
	switch (index) {
	case 1:
		brightness1 = value;
		sendToBroker("report/percentage1", val);
		break;
	case 2:
		brightness2 = value;
		sendToBroker("report/percentage2", val);
		break;
	case 3:
		brightness3 = value;
		sendToBroker("report/percentage3", val);
		break;
	case 4:
		brightness4 = value;
		sendToBroker("report/percentage4", val);
		break;
	default:
		break;
	}
