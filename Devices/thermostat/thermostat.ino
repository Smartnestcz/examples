...
#define FIRMWARE_VERSION "Example-thermostat"  // Custom name for this program
...
void turnOff() {
	...
	if (client.connected()) {
		sendToBroker("report/temperature", "OFF");
	}
}
...
void turnOn() {
	...
	if (client.connected()) {
		sendToBroker("report/temperature", "ON");
	}
}

void setSetpoint(double temp) {
	Serial.printf("setpoint changed to %3.2f", temp);
	//Do something with the value, Turn on Heater?
}
